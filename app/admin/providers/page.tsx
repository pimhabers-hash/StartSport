  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="card-surface rounded-2xl overflow-hidden mb-8">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {providers.length === 0 && (
              <tr><td colSpan={4} className="px-5 py-8 text-center text-brand-muted font-mono text-sm">Nog geen aanbieders.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div className="card-surface rounded-2xl p-6 space-y-4">
        ...
        <FormVeld label="Naam" naam="naam" verplicht waarde={naam} onChange={(v) => handleNaam(v as string)} placeholder="bijv. Decathlon" />
        <FormVeld label="Slug" naam="slug" verplicht waarde={slug} onChange={(v) => setSlug(v as string)} />
        <FormVeld
          label="Affiliate netwerk"
          naam="netwerk"
          type="select"
          waarde={netwerk}
          onChange={(v) => setNetwerk(v as string)}
          opties={[
            { label: "Direct", waarde: "direct" },
            { label: "Awin", waarde: "Awin" },
            { label: "Daisycon", waarde: "Daisycon" },
            { label: "TradeTracker", waarde: "TradeTracker" },
          ]}
          hulptekst="Het affiliate netwerk waarmee je samenwerkt voor commissie"
        />
